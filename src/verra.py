...
def background(verra):
    def wrapped(*args, **kwargs):
        return asyncio.get_event_loop().run_in_executor(None, verra, *args, **kwargs)
    return wrapped

@background
def verra(account, driver, today, download_folder):
...
def main():
    ...
    for account in accounts:
        ...
        locals()[account] = webdriver.Chrome(options=options)
        locals()[account].implicitly_wait(15)
        verra(account, locals()[account], today, download_folder)
